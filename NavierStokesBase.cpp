...
int  NavierStokesBase::liquid_interval    = -1; // Mu-Hua Liquid output
...
NavierStokesBase::Initialize ()
{
    ...
    pp.query("liquid_interval",liquid_interval);
...
NavierStokesBase::post_timestep (int crse_iteration)
{
    ...
    //
    // Derive Moist statistics
    //
    if (level==0 && liquid_interval>0 && (parent->levelSteps(0)%liquid_interval == 0))
    {
        sum_liquid_quantities();
    }
#if (BL_SPACEDIM==3)
    //
...
NavierStokesBase::sum_liquid_quantities ()
{
	...
	Real* levLiquid = new Real[liquidVars*levKsize];
	...
	for (int l=0, k=0; l<levKsize; l++)
		...
		for (int v=0; v<liquidVars; v++)
		    liquid[k*liquidVars+v] += levLiquid[l*liquidVars+v];
	...
	std::string DirPath = "LiquidData";
        ...
        for (int k=0; k<ksize; k++)
        {
            ...
            for (int v=0; v<liquidVars; v++)
                fprintf(file,"%e ",liquid[k*liquidVars+v]);
            fprintf(file,"\n");
        }
        fclose(file);
